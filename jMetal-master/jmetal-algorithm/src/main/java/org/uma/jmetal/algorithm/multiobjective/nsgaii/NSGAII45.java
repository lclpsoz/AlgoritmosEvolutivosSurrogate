...
/**
 * Implementation of NSGA-II following the scheme used in jMetal4.5 and former
 * versions, i.e, without implementing the {@link AbstractGeneticAlgorithm}
 * interface.
 *
...
public class NSGAII45<S extends Solution<?>> implements Algorithm<List<S>> {
	...
	protected List<S> evaluatePopulation(List<S> population) {
		...
		if (treinou) {
			...
			user userObject = new user(getName(), "treino", Solucoes, Objetivos);
		...
		/*
		 * appendToFile("ObjetivosSurrogate:", "out.txt"); if (ObjetivosSurrogate !=
		 * null) appendToFile(toJsonString(ObjetivosSurrogate), "out.txt"); else {
		 * //System.out.println("NULL"); appendToFile("NULL", "out.txt"); }
		 */
		return population;
	}
